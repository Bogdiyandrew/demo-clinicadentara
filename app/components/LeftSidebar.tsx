    ...
    <aside className="hidden lg:flex lg:flex-col lg:w-72 xl:w-80 h-screen bg-linear-to-b from-slate-950 via-slate-900 to-slate-950 text-gray-300 sticky top-0 border-r border-white/5 shadow-2xl">
      ...
      <div className="relative flex flex-col h-full p-6">
          ...
          <div className="space-y-3">
            {contactInfo.map((info, index) => (
              <div key={index} className="flex items-center gap-3 text-sm">
                <info.icon className="w-4 h-4 text-sky-400 hrink-0" />
                {info.href ? (
                  <a 
                    href={info.href} 
        ...
        >
          <Link
            href="/programari"
            className="group relative block w-full px-6 py-4 text-center text-base font-bold text-white bg-linear-to-r from-sky-500 to-blue-600 rounded-2xl shadow-lg shadow-sky-500/50 hover:shadow-sky-500/80 transition-all duration-300 hover:scale-105 overflow-hidden"
          >
