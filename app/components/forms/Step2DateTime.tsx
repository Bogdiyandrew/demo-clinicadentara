...
export default function Step2DateTime({ onNext, onBack, onUpdateData, formData }: Props) {
  ...
  const footer = date ? (
    ...
    <p className="mt-4 text-center text-gray-600">Vă rugăm selectați o zi.</p> 
  );
        ...
        <div className="flex justify-center">
          <DayPicker
            ...
            classNames={{
              ...
              head_cell: 'text-gray-700 font-semibold uppercase', 
              head: 'border-t-0', 
              day: 'w-10 h-10 transition-colors rounded-full hover:bg-sky-100 text-gray-900', 
              ...
              {timeSlots.map((time) => (
                <button
                  ...
                  className={`
                    ...
                    ${timeSlot === time 
                      ? 'border-sky-600 bg-sky-50 shadow-md text-sky-700' 
                      : 'border-gray-300 bg-gray-50 text-gray-900 hover:bg-white' 
                    }
                  `}
                >
                  {time}
